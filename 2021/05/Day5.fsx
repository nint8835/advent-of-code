...
let rec calculateHits
    (allowDiags: bool)
    (hits: int[,])
    (remainingLineEnds: int[][] list)
    =
    ...
    | hit :: tail ->
        ...
        if startY = endY then
            ...
            |> Array.iter (fun (x: int) ->
                hits.[x, startY] <- hits.[x, startY] + 1)
        else if startX = endX then
            [| Math.Min(startY, endY) .. Math.Max(startY, endY) |]
            |> Array.iter (fun (y: int) ->
                hits.[startX, y] <- hits.[startX, y] + 1)
