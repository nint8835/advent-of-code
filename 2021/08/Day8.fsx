...
let getIntersectionCount (a: Set<'T>) (b: Set<'T>) : int =
...
let determineRemainingDigits
    (definiteDigits: DigitValues)
    (allVals: string[])
    : DigitValues =
    let one = definiteDigits |> Map.findKey (fun _ v -> v = 1)

    let four = definiteDigits |> Map.findKey (fun _ v -> v = 4)

    let seven = definiteDigits |> Map.findKey (fun _ v -> v = 7)

    let eight = definiteDigits |> Map.findKey (fun _ v -> v = 8)
    ...
    |> Array.map (fun value ->
        (value,
         ...
         |> String.concat ""
         |> int))
    |> Array.map (fun (charSet, intersections) ->
        ...
        | _ ->
            failwithf
                $"Unexpected intersection number %d{intersections} for charset %A{charSet}")
    |> Map
...
let calculateDigitValues (row: string[][]) : (int * int) =
    ...
    let digitValues = determineRemainingDigits definiteDigits (Array.concat row)
