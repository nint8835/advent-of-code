let inputData =
    (System.IO.File.ReadAllText "Day4/input.txt").Split "\n"
    |> Array.map (fun line ->
        line.Split "," |> Array.map (fun elf -> elf.Split "-" |> Array.map int))
...
let partAResult =
    inputData |> Array.filter (tryBothOrders partA) |> Array.length

let partBResult =
    inputData |> Array.filter (tryBothOrders partB) |> Array.length
